...
database = pd.read_csv("database_with_news.csv", sep=";", header = 0 )
...
list_of_files = glob.glob("final csv/*.csv")
for file in sorted(list_of_files):
    ...
    #variables needed 
    error1 = 0.3        #default mass error, in Da
    first_peak = 0  
    mass_error = 0
    window = 0.2        
    last_peak = 0
...
df_glycan_final.to_csv("results_new_masses.csv", index = False)

print(df_glycan_final)
